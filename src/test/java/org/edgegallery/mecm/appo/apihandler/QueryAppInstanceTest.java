...
public class QueryAppInstanceTest {
    ...
    private void createAppInstanceFlowUrls(MockRestServiceServer server) throws Exception {
        ...
        resetServer(server).expect(requestTo(url)).andExpect(method(HttpMethod.GET)).andRespond(
        ...
        resetServer(server).expect(requestTo(url)).andExpect(method(HttpMethod.GET))
        ...
        resetServer(server).expect(requestTo(url)).andExpect(method(HttpMethod.POST)).andRespond(withSuccess());
    }
    ...
    private void syncAppInstanceInfos(MockRestServiceServer server) throws Exception {
        ...
        url = "http://1.1.1.1:10000/lcmcontroller/v1/tenants/" + TENANT_ID + "/app_instances/" + "sync_deleted";
        server.expect(requestTo(url)).andExpect(method(HttpMethod.GET)).andRespond(withSuccess());

        // Mocking get updated app instance infos API
        url = "http://1.1.1.1:10000/lcmcontroller/v1/tenants/" + TENANT_ID + "/app_instances/" + "sync_updated";
        server.expect(requestTo(url)).andExpect(method(HttpMethod.GET)).andRespond(withSuccess());
    }
    ...
    public void appInstanceProcessFlowFailureTest() throws Exception {
        ...
        getResult.andDo(MockMvcResultHandlers.print()).andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
