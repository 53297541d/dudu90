...
public class AppoHandlerTest {
    ...
    private void createAppInstanceFlowUrls(MockRestServiceServer server) throws Exception {
        ...
        url = "http://10.9.9.1:11111/inventory/v1/mepms/1.1.1" + ".1";
        server.expect(requestTo(url)).andExpect(method(HttpMethod.GET)).andRespond(
            withSuccess("{\"mepmIp\":\"1.1.1.1\",\"mepmPort\":\"10000\",\"userName\":\"Test\"}",
        ...
        url = "http://10.9.9.1:11111/inventory/v1/tenants/" + TENANT_ID + "/mechosts/" + "1.1.1.1" + "/apps";
    ...
    private void instantiateAppInstanceFlowUrls(MockRestServiceServer server, String appInstanceId) {
        ...
        url = "http://1.1.1.1:10000/lcmcontroller/v2/tenants/" + TENANT_ID + APP_INSTANCE + appInstanceId
            + "/instantiate";
        server.expect(requestTo(url)).andExpect(method(HttpMethod.POST)).andRespond(withSuccess());
    ...
    private void deleteAppInstanceFlowUrls(MockRestServiceServer server, String appInstanceId) {
        ...
        url = "http://10.9.9.1:11111/inventory/v1/tenants/" + TENANT_ID + "/mechosts/" + "1.1.1.1" + "/apps/"
            + appInstanceId;
        server.expect(requestTo(url)).andExpect(method(HttpMethod.DELETE)).andRespond(withSuccess());
    }
    ...
    public void instantiateTerminateTest() throws Exception {
        ...
        ResultActions postResult = mvc.perform(MockMvcRequestBuilders.post(APPO_TENANT + TENANT_ID + "/app_instances")
            .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).with(csrf()).content(
                "{ \"appPackageId\": \"f20358433cf8eb4719a62a49ed118c9b\", \"appName\": \"face_recognition-1\", "
                    ...
                    + "\"appInstanceDescription\": \"face_recognition\", " + "\"mecHost\": \"1.1.1.1\" }")
        ...
        ResultActions getAllResult = mvc.perform(
            MockMvcRequestBuilders.get(APPO_TENANT + TENANT_ID + "/app_instance_infos")
                .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON_VALUE).with(csrf()));
        ...
        Assert.assertEquals("{\"response\":[{\"appInstanceId\":\"test-required\","
            ...
            getAllResponse);
        // Get application instance id
        ResultActions getResult = mvc.perform(
            MockMvcRequestBuilders.get(APPO_TENANT + TENANT_ID + "/app_instance_infos/" + appInstanceId)
                .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON_VALUE).with(csrf()));
        ...
        Assert.assertEquals("{\"response\":{\"appInstanceId\":\"" + appInstanceId + "\","
            ...
            + "\"appId\":\"f50358433cf8eb4719a62a49ed118c9b\",\"appDescriptor\":\"face_recognition\","
            + "\"mecHost\":\"1.1.1.1\",\"mepmHost\":\"1.1.1.1\",\"operationalStatus\":\"Created\","
            + "\"operationInfo\":\"success\"}}", getResponse);
        ...
        ResultActions postInstantiateResult = mvc.perform(
            MockMvcRequestBuilders.post(APPO_TENANT + TENANT_ID + APP_INSTANCE + appInstanceId)
                .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON_VALUE).with(csrf()).content(
        ...
        // delete
        ResultActions deleteResult = mvc.perform(
            MockMvcRequestBuilders.delete(APPO_TENANT + TENANT_ID + APP_INSTANCE + appInstanceId)
                .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).with(csrf())
                .header(ACCESS_TOKEN, SAMPLE_TOKEN));
        deleteResult.andDo(MockMvcResultHandlers.print()).andExpect(MockMvcResultMatchers.status().is2xxSuccessful())
            .andReturn();
